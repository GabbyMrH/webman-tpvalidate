...
namespace Gabbymrh\WebmanTpvalidate;

use Closure;
use support\Db;
use Gabbymrh\WebmanTpvalidate\exception\ValidateException;
use support\Model;
use think\helper\Str;
use Gabbymrh\WebmanTpvalidate\validate\ValidateRule;
...
class Validate
{
    ...
    public function unique($value, $rule, array $data = [], string $field = ''): bool
    {
        ...
        $pk = !empty($rule[3]) ? $rule[3] : Model::from($db)->first()->getKeyName();
        if (is_string($pk)) {
            if (isset($rule[2])) {
                $map[] = [$pk, '<>', $rule[2]];
        ...
        if ($db->where($map)->select($pk)->count() >= 1) {
            return false;
        }
